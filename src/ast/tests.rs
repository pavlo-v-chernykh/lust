...
fn test_expand_number() {
    ...
    let num = 1_f64;
    assert_eq!(e_number!(num), e_number!(num).expand(state).ok().unwrap());
}
...
fn test_expand_string() {
    ...
    assert_eq!(e_string!(s), e_string!(s).expand(state).ok().unwrap());
}
...
fn test_expand_symbol() {
    let ref mut state = State::new("user".to_string());
    let s = "+";
    assert_eq!(e_symbol!(s), e_symbol!(s).expand(state).ok().unwrap());
}
...
fn test_expand_keyword() {
    let ref mut state = State::new("user".to_string());
    let s = ":+";
    assert_eq!(e_keyword!(s), e_keyword!(s).expand(state).ok().unwrap());
}

#[test]
fn test_expand_empty_list() {
    let ref mut state = State::new("user".to_string());
    assert_eq!(e_list![], e_list![].expand(state).ok().unwrap());
}
...
fn test_expand_fn() {
    ...
    assert_eq!(e, n.expand(state).ok().unwrap());
}
...
fn test_expand_quote() {
    ...
    assert_eq!(e_call!["quote", e_symbol!["a"]], n.expand(state).ok().unwrap());
...
fn test_expand_unquote() {
    ...
    assert_eq!(expected_result, n.expand(state).ok().unwrap());
}
...
fn test_eval_number_to_itself() {
    ...
    let actual_result = e_number!(num).eval(state);
...
fn test_eval_keyword_to_itself() {
    ...
    let actual_result = e_keyword!(keyword).eval(state);
    assert_eq!(expected_result, actual_result.ok().unwrap());
}
...
fn test_eval_string_to_itself() {
    ...
    let actual_result = e_string!(s).eval(state);
    assert_eq!(expected_result, actual_result.ok().unwrap());
}
...
fn test_eval_undefined_symbol_to_error() {
    ...
    let actual_result = e_symbol!("a").eval(state);
...
fn test_eval_true_to_matching_bool() {
    ...
    let actual_result = e_symbol!("true").eval(state);
    assert_eq!(expected_result, actual_result.ok().unwrap());
}
...
fn test_eval_false_to_matching_bool() {
    ...
    let actual_result = e_symbol!("false").eval(state);
    assert_eq!(expected_result, actual_result.ok().unwrap());
}
...
fn test_eval_nil_to_empty_list() {
    ...
    let actual_result = e_symbol!("nil").eval(state);
    assert_eq!(expected_result, actual_result.ok().unwrap());
}
...
fn test_eval_def_special_form() {
    ...
    let actual_result = actual_input.eval(state);
    assert_eq!(expected_result, actual_result.ok().unwrap());
}
...
fn test_eval_let_special_form() {
    ...
    let let_expr = e_let![[e_symbol!["a"], e_call!["+", e_number![1.], e_number![2.]],
                           e_symbol!["b"], e_call!["+", e_symbol!["a"], e_number![3.]]],
                          e_call!["+", e_symbol!["a"], e_symbol!["b"]]];
    assert_eq!(e_number![9.], let_expr.eval(state).ok().unwrap());
}
...
fn test_eval_plus_builtin_fn() {
    ...
    assert_eq!(e_number!(6_f64), actual_input.eval(state).ok().unwrap());
}
...
fn test_eval_quote_builtin_fn() {
    ...
    assert_eq!(expected_result, expr.eval(state).ok().unwrap());
}
